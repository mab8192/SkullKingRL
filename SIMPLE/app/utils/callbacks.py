...
class SelfPlayCallback(EvalCallback):
    def __init__(self, opponent_type, threshold, env_name, *args, **kwargs):
        ...
        if self.opponent_type == 'rules':
            # the threshold is the overall best evaluation by the agent against a rules-based agent
            self.threshold = bmr
        else:
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # this will set self.best_mean_reward to the reward from the evaluation as it's previously -np.inf
            ...
            av_timesteps = np.mean(
                MPI.COMM_WORLD.allgather(self.num_timesteps))
            total_episodes = np.sum(
                MPI.COMM_WORLD.allgather(self.n_eval_episodes))
            ...
            if result and av_reward > self.threshold:
                ...
                if rank == 0:  # write new files
                    ...
                    if self.callback is not None:
                        av_rules_based_reward_str = str(
                            round(av_rules_based_reward, 3))
                    else:
                    ...
                    target_file = os.path.join(
                        self.model_dir,  f"_model_{generation_str}_{av_rules_based_reward_str}_{av_rewards_str}_{str(self.base_timesteps + self.num_timesteps)}_.zip")
                    copyfile(source_file, target_file)
                    target_file = os.path.join(
                        self.model_dir,  f"best_model.zip")
                    copyfile(source_file, target_file)
